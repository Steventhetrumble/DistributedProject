...
class Comparison extends Component {
  ...
  async componentDidMount(){
    try{
      const tempmodel = await tf.loadModel(tf.io.browserHTTPRequest('http://127.0.0.1:8080/myview/method1/model'))
      this.setState({model:tempmodel })

      const res = await fetch('http://127.0.0.1:8080/myview/method2/')
      var tempX = [];
      var tempY = [];
  ...
  async trainModel(){
    ...
    const resultOfSave = await this.state.model.save(tf.io.browserHTTPRequest('http://127.0.0.1:8080/myview/method3/'));
